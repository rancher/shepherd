...
// RegisterReplicaSetStatusHandler configures a ReplicaSetController to execute a ReplicaSetStatusHandler for every events observed.
...
// RegisterReplicaSetGeneratingHandler configures a ReplicaSetController to execute a ReplicaSetGeneratingHandler for every events observed, passing the returned objects to the provided apply.Apply.
// If a non-empty condition is provided, it will be updated in the status conditions for every handler execution
...
// Handle executes the configured ReplicaSetGeneratingHandler and pass the resulting objects to apply.Apply, finally returning the new status of the resource
...
func (a *replicaSetGeneratingHandler) isNewResourceVersion(obj *v1.ReplicaSet) bool {
	if !a.opts.UniqueApplyForResourceVersion {
		return true
	}
...
// Only used if UniqueApplyForResourceVersion is set in generic.GeneratingHandlerOptions
func (a *replicaSetGeneratingHandler) storeResourceVersion(obj *v1.ReplicaSet) {
	if !a.opts.UniqueApplyForResourceVersion {
		return
	}
