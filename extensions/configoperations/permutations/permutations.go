...
func Permute(permutations []Permutation, baseConfig map[string]any) ([]map[string]any, error) {
	...
	for _, keyPathValue := range permutations[0].KeyPathValues {
		permutedConfig, err := configoperations.DeepCopyMap(baseConfig)
		if err != nil {
			return nil, err
		}

		permutedConfig, err = configoperations.ReplaceValue(permutations[0].KeyPath, keyPathValue, permutedConfig)
		if err != nil {
			return nil, err
		}
