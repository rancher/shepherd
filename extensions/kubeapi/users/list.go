...
func ListUsers(client *rancher.Client, listOpt metav1.ListOptions) (*v3.UserList, error) {
	...
	for _, unstructuredUser := range unstructuredList.Items {
		user := &v3.User{}
		err := scheme.Scheme.Convert(&unstructuredUser, user, unstructuredUser.GroupVersionKind())
		if err != nil {
			return nil, err
		}
...
func ListUserAttributes(client *rancher.Client, listOpt metav1.ListOptions) (*v3.UserAttributeList, error) {
	...
	for _, unstructuredUserAttribute := range unstructuredList.Items {
		userAttributes := &v3.UserAttribute{}
		err := scheme.Scheme.Convert(&unstructuredUserAttribute, userAttributes, unstructuredUserAttribute.GroupVersionKind())
		if err != nil {
			return nil, err
		}
