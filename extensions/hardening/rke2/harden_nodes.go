...
func HardenNodes(nodes []*nodes.Node, nodeRoles []string) error {
	...
	for key, node := range nodes {
		...
		if strings.Contains(nodeRoles[key], "--controlplane") {
			...
			dirPath := filepath.Join(user.HomeDir, "go/src/github.com/rancher/shepherd/extensions/hardening/rke2")
