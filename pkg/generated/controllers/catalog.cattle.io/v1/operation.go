...
// Handle executes the configured OperationGeneratingHandler and pass the resulting objects to apply.Apply, finally returning the new status of the resource
...
func (a *operationGeneratingHandler) isNewResourceVersion(obj *v1.Operation) bool {
	if !a.opts.UniqueApplyForResourceVersion {
		return true
	}
...
// Only used if UniqueApplyForResourceVersion is set in generic.GeneratingHandlerOptions
func (a *operationGeneratingHandler) storeResourceVersion(obj *v1.Operation) {
	if !a.opts.UniqueApplyForResourceVersion {
		return
	}
