...
type RKEBootstrapClient interface {
	generic.ClientInterface[*v1.RKEBootstrap, *v1.RKEBootstrapList]
}
...
// RKEBootstrapStatusHandler is executed for every added or modified RKEBootstrap. Should return the new status to be updated
type RKEBootstrapStatusHandler func(obj *v1.RKEBootstrap, status v1.RKEBootstrapStatus) (v1.RKEBootstrapStatus, error)

// RKEBootstrapGeneratingHandler is the top-level handler that is executed for every RKEBootstrap event. It extends RKEBootstrapStatusHandler by a returning a slice of child objects to be passed to apply.Apply
...
// Handle executes the configured RKEBootstrapGeneratingHandler and pass the resulting objects to apply.Apply, finally returning the new status of the resource
...
func (a *rKEBootstrapGeneratingHandler) isNewResourceVersion(obj *v1.RKEBootstrap) bool {
	if !a.opts.UniqueApplyForResourceVersion {
		return true
	}
...
// Only used if UniqueApplyForResourceVersion is set in generic.GeneratingHandlerOptions
func (a *rKEBootstrapGeneratingHandler) storeResourceVersion(obj *v1.RKEBootstrap) {
	if !a.opts.UniqueApplyForResourceVersion {
		return
	}
