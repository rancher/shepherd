...
// Handle executes the configured ClusterLoggingGeneratingHandler and pass the resulting objects to apply.Apply, finally returning the new status of the resource
...
func (a *clusterLoggingGeneratingHandler) isNewResourceVersion(obj *v3.ClusterLogging) bool {
	if !a.opts.UniqueApplyForResourceVersion {
		return true
	}
...
// Only used if UniqueApplyForResourceVersion is set in generic.GeneratingHandlerOptions
func (a *clusterLoggingGeneratingHandler) storeResourceVersion(obj *v3.ClusterLogging) {
	if !a.opts.UniqueApplyForResourceVersion {
		return
	}
