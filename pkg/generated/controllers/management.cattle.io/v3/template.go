...
// Handle executes the configured TemplateGeneratingHandler and pass the resulting objects to apply.Apply, finally returning the new status of the resource
...
func (a *templateGeneratingHandler) isNewResourceVersion(obj *v3.Template) bool {
	if !a.opts.UniqueApplyForResourceVersion {
		return true
	}
...
// Only used if UniqueApplyForResourceVersion is set in generic.GeneratingHandlerOptions
func (a *templateGeneratingHandler) storeResourceVersion(obj *v3.Template) {
	if !a.opts.UniqueApplyForResourceVersion {
		return
	}
